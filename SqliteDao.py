...
class SqliteDao:
    ...
    # ----------------------
    def create_recent_entries_db(self):
        self.open()
        c = self.__con.cursor()
        c.execute('DROP TABLE IF EXISTS RecentPullRequests')
        c.execute('CREATE TABLE RecentPullRequests(PR int not null, branch text, date text, eDate float, '
        ...
        c.execute('INSERT INTO RecentPullRequests SELECT t1.* FROM PullRequests t1 '
                  'WHERE t1.date = (SELECT MAX(t2.date) FROM PullRequests t2 '
                  'WHERE t2.branch = t1.branch AND t2.date IS NOT "N/A") '
                  'AND t1.eDate = (SELECT MAX(t2.eDate) FROM PullRequests t2 '
                  'WHERE t2.branch = t1.branch AND t2.eDate IS NOT 0.0)')
        self.close()

    def get_reports(self):
        self.open()
        c = self.__con.cursor()
        c.execute('SELECT branch, date, report FROM RecentPullRequests')
        data = c.fetchall()
        self.close()
        reports = {}
        if data:
